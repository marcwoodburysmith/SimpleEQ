...
void SimpleEQAudioProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
    ...
    *leftChain.get<ChainPositions::Peak>().coefficients = *peakCoefficients; //get second in leftChain list i.e. 'Filter' and set these to params values
    *rightChain.get<ChainPositions::Peak>().coefficients = *peakCoefficients;
    ...
    switch( chainSettings.lowCutSlope )
    {
        case Slope_12:
        {
            *leftLowCut.get<0>().coefficients = *cutCoefficients[0]; //ie leftLowCut get first filter in that chain
...
{
    //Note: update parameters before you process audio
...
juce::AudioProcessorValueTreeState::ParameterLayout SimpleEQAudioProcessor::createParameterLayout()
{
    ...
    layout.add(std::make_unique<juce::AudioParameterFloat>("Peak Gain",
                                                           "Peak Gain",
                                                           juce::NormalisableRange<float>(-24.0f, 24.f, 0.5f, 0.1f),
                                                           0.0f));
    
    layout.add(std::make_unique<juce::AudioParameterFloat>("Peak Quality",
                                                           "Peak Quality",
                                                           juce::NormalisableRange<float>(0.1f, 10.f, 0.05f, 0.1f),
                                                           1.0f));
